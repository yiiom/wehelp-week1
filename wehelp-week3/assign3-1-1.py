...
file=open("hotels.csv","w",encoding="utf-8")
file.write("ChineseName,EnglishName,ChineseAddress,EnglishAddress,Phone,RoomCount\n")
...
for i in range(len(data1["list"])):   #會重複執行 len(data1["list"]) 次
    ...
    file.write(chinese_hotel["旅宿名稱"]+","+english_hotel["hotel name"]+","+chinese_hotel["地址"]+","+english_hotel["address"]+","+chinese_hotel["電話或手機號碼"]+","+chinese_hotel["房間數"]+"\n")
    print(
